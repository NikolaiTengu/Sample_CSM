...
export default function RecentFeedback({ responses }: RecentFeedbackProps) {
  const getRatingColor = (rating: number) => {
    ...
    if (rating >= 3.5) return 'bg-[rgba(230,200,80,0.1)] text-[rgb(230,200,80)] border-[rgb(230,200,80)]'
    ...
    <div className="scp-card p-6">
      ...
      <div className="space-y-4">
        {responses.map((response, idx) => (
          <div
            key={response.id}
            className="border-2 border-[rgb(100,100,100)] bg-[rgba(28,28,28,0.5)] p-5 transition-all duration-300 hover:border-[rgb(230,200,80)] hover:shadow-[0_0_20px_rgba(230,200,80,0.3)]"
          >
            <div className="flex items-start justify-between mb-3 flex-wrap gap-3">
              <div>
                <div className="flex items-center gap-3 mb-1">
                  <span className="text-[rgb(230,200,80)] font-bold text-xs">ENTRY #{String(idx + 1).padStart(3, '0')}</span>
            ...
            <div className="my-3 border-l-4 border-[rgb(230,200,80)] pl-4 bg-[rgba(230,200,80,0.05)] py-2">
            ...
            <div className="mt-4 pt-3 border-t border-[rgb(100,100,100)] grid grid-cols-2 md:grid-cols-4 gap-3">
              <div className="text-center">
                ...
                <span className="font-bold text-[rgb(230,200,80)] text-sm">{response.ratings.timeliness}/5</span>
              </div>
              <div className="text-center">
                <p className="text-xs text-[rgb(160,160,160)] mb-1 tracking-wider">QUALITY</p>
                <span className="font-bold text-[rgb(230,200,80)] text-sm">{response.ratings.quality}/5</span>
              </div>
              <div className="text-center">
                <p className="text-xs text-[rgb(160,160,160)] mb-1 tracking-wider">PROFESSIONALISM</p>
                <span className="font-bold text-[rgb(230,200,80)] text-sm">{response.ratings.professionalism}/5</span>
              </div>
              <div className="text-center">
                <p className="text-xs text-[rgb(160,160,160)] mb-1 tracking-wider">COMMUNICATION</p>
                <span className="font-bold text-[rgb(230,200,80)] text-sm">{response.ratings.communication}/5</span>
              </div>
            </div>
          </div>
        ))}
